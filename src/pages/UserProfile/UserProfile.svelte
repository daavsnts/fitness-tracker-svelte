<script lang="ts">
  ...
  const { getTotalWaterIntake } = waterTrackerStore;
  const { getTotalExercisePauses } = exerciseTrackerStore;

  let totalWaterIntake = getTotalWaterIntake();
  let totalExercisePauses = getTotalExercisePauses();
...
<div class="UserProfile">
  <div class="statistics">
    <div class="water-intake-container">
      <h1>Total water intake:</h1>
      <h2>{$totalWaterIntake}ml</h2>
    </div>

    <div class="exercise-container">
      <h1>Total exercises:</h1>
      <h2>{$totalExercisePauses}</h2>
    </div>
  </div>
